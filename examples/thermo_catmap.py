...
from aiida.orm import SinglefileData, List, Dict, Int, Float, Str, Bool  # pylint: disable=unused-import
...
def test_run(catmap_code):
    ...
    SinglefileData = DataFactory('singlefile')  # pylint: disable=redefined-outer-name, invalid-name
    energies = SinglefileData(file=path.join(INPUT_DIR, 'energies.txt'))
    ...
    inputs = {
        'electrocatal':
        Bool(False),  # Thermal catalysis run
        'energies':
        energies,  # File with all energies
        'code':
        catmap_code,  # Code to run with
        'rxn_expressions':
        List(list=[
            '*_s + CO_g -> CO*',
            '2*_s + O2_g <-> O-O* + *_s -> 2O*',
    ...
    engine.submit(CalculationFactory('catmap'), **inputs)
